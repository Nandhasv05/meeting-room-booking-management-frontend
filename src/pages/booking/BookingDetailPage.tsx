...
import { TimePoint } from '../../components/booking/TimePoint';
...
import {
  cancelBookingResponseResetStart,
  cancelBookingStart,
  deleteBookingResponseResetStart,
  deleteBookingStart,
  fetchAttendeesStart,
  fetchBookingStart,
} from '../../redux/bookings/bookings.action';
import {
  selectAttendees,
  selectBooking,
  ...
  selectDeleteBookingResponse,
} from '../../redux/bookings/bookings.selector';
import { useReduxResponse } from '../../redux/_common/useReduxResponse';
import { Guest, durationLabel, initials } from '../../helpers/booking/bookingValidation';
...
export function BookingDetailPage() {
  ...
  /******* SELECTORS *******/
  const data = useAppSelector(selectBooking) as Booking | null;
  const isLoading = useAppSelector(selectBookingLoading);
  const guests = useAppSelector(selectAttendees) as Guest[] | undefined;
  const cancelPending = useAppSelector(selectCancelBookingLoading);
  const deletePending = useAppSelector(selectDeleteBookingLoading);
  const cancelResponse = useAppSelector(selectCancelBookingResponse);
  const deleteResponse = useAppSelector(selectDeleteBookingResponse);

  /******* EFFECTS *******/
  useEffect(() => {
    if (!id) return;
    dispatch(fetchBookingStart({ id }));
    dispatch(fetchAttendeesStart({ id }));
  }, [id, dispatch]);

  /******* HANDLERS *******/
  const resetCancel = useCallback(() => dispatch(cancelBookingResponseResetStart()), [dispatch]);
  const resetDelete = useCallback(() => dispatch(deleteBookingResponseResetStart()), [dispatch]);
  useReduxResponse(cancelResponse, resetCancel, () => {
  ...
  useReduxResponse(deleteResponse, resetDelete, () => {
        ...
        <div className="grid lg:grid-cols-[10.5rem_minmax(0,1fr)]">
          ...
          <div className="p-5 sm:p-6">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {canDelete ? (
                <DangerButton type="button" disabled={deletePending} onClick={() => setConfirmDelete(true)}>
                  <Trash2 className="h-4 w-4" />
                  Delete booking
                </DangerButton>
              ) : null}
              {canCancel ? (
                <DangerButton type="button" disabled={cancelPending} onClick={() => setConfirmCancel(true)}>
                  Cancel booking
                </DangerButton>
              ) : null}
              {data.EventId ? (
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <DangerButton
              ...
              onClick={() => id && dispatch(cancelBookingStart({ id, reason: cancelReason.trim() }))}
            >
              {cancelPending ? 'Cancelling…' : 'Yes'}
            </DangerButton>
          </div>
        }
      >
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <DangerButton
              ...
              onClick={() => id && dispatch(deleteBookingStart({ id }))}
            >
